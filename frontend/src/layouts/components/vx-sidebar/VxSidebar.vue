                ...
                <div class="header-sidebar flex items-end justify-between" slot="header">
                    ...
                    <div>
                        <template v-if="showCloseButton">
                            <feather-icon icon="XIcon" :style="{paddingBottom: '20px'}" class="m-0 cursor-pointer" @click="$store.commit('TOGGLE_IS_SIDEBAR_ACTIVE', false)"></feather-icon>
                        </template>
                        <template v-else-if="!showCloseButton && !sidebarItemsMin">
                            <feather-icon icon="DiscIcon" class="mr-0 cursor-pointer" :style="{paddingBottom: '20px'}" svg-classes="stroke-current" v-show="!reduce" @click="toggleReduce(true)" id="btnSidebarToggler"></feather-icon>
                            <feather-icon icon="CircleIcon" class="mr-0 cursor-pointer" :style="{paddingBottom: '20px'}" svg-classes="stroke-current" v-show="reduce" @click="toggleReduce(false)" id="btnSidebarToggler"></feather-icon>
                        </template>
                    </div>
                </div>
...
import { TOGGLE_IS_SIDEBAR_ACTIVE,
...
export default {
    ...
    computed: {
        isSidebarActive: {
            get() {
                return this.$store.state.layout.isSidebarActive
            },
            set(val) {
        ...
        reduceButton: {
            get() {
                return this.$store.state.layout.reduceButton;
            },
            set(val) {
        ...
        sidebarItemsMin() {
            return this.$store.state.layout.sidebarItemsMin;
        },
        isGroupActive() {
            return (sidebarItem) => {
